...
/**
 * احجزلي — idempotent database bootstrap.
 *
 * Creates roles, permissions, default categories, languages, countries,
 * cities and a seed super-admin account. Safe to run multiple times
 * (every step is an upsert). Used by both the CLI seed (`npm run db:seed`)
 * and the first-run provisioning hook on serverless deployments.
 */
...
export const SEED_ADMIN_EMAIL = "admin@ahjezli.app";
